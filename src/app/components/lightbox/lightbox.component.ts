...
export class LightboxComponent implements OnInit, AfterViewInit, OnDestroy, OnInit {
  ...
  ngOnInit(): void {
    this.album.forEach(album => {
      if (album.title) {
        album.title = this._sanitizer.sanitize(SecurityContext.HTML, album.title);
      }
    });
  }
  ...
  public isPropertyDefined(property: any): boolean {
  ...
  public fullscreen(index: number): void {
    this._fullScreenService.open(this.album, index);
  }
